...
public class YuvFrame {
    ...
    public float[] getYFloat() {
        ...
        for (int y = 0; y < height; y++) {
            if (width >= 0) {
                System.arraycopy(nv21Buffer, y * width, bytes, y * width, width);
            }
        }
    ...
    private void copyPlanes(final VideoFrame.Buffer videoFrameBuffer) {
        ...
        synchronized (planeLock) {
            ...
            for (int y = 0; y < height; y++) {
                if (width >= 0) {
                    System.arraycopy(yPlane, y * yStride, nv21Buffer, y * width, width);
                }
            }
    ...
    private void copyPlanesCropped(final VideoFrame.Buffer videoFrameBuffer) {
        ...
        synchronized (planeLock) {
            ...
            final VideoFrame.Buffer croppedVideoFrameBuffer;
            if (width > height) {
                croppedVideoFrameBuffer = videoFrameBuffer.cropAndScale((width - height) / 2, 0, height, height, height, height);
            } else {
                croppedVideoFrameBuffer = videoFrameBuffer.cropAndScale(0, (height - width) / 2, width, width, width, width);
            }
