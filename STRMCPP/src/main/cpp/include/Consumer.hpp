...
public:
    virtual void onProcessEnd(const T& item) = 0;
};
...
private:
    ...
    std::thread mThread;
    const ConsumerCallback<T>* mConsumerCallback;

    const int mMaxNumItems;
    const std::queue<T> mItems;
    const std::condition_variable mItemsCV;
    std::mutex mItemsMtx;
};
