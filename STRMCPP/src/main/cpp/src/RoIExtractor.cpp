...
bool RoIExtractor::useOpticalFlowRoIs() {
    return mConfig.OF_ROI;
}

void RoIExtractor::process(const std::pair<std::pair<Frame, Frame>, std::vector<BoundingBox>>& item) {
...
void RoIExtractor::mergeSingleFrameRoIs(const Frame& frame, std::vector<RoI> rois, float mergeThreshold) {
    while (true) {
        bool updated = false;
...
std::vector<RoI> RoIExtractor::getOpticalFlowRoIs(const Frame& prevFrame, const Frame& currFrame,
...
std::vector<RoI> RoIExtractor::getPixelDiffRoIs(const Frame& prevFrame, const Frame& currFrame, const cv::Size& targetSize) {
